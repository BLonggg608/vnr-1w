...
export default function KhaiQuat() {
  ...
  return (
    <div className="w-full min-h-full flex flex-col items-center p-2 relative font-sans transition-all duration-700 ease-in-out">
      {/* Tắt h-full ép cứng để cho ContentArea tự xuất scrollbar nếu thu bé, mở lên min-h-full để luôn ra giữa nếu màn to */}
      <div className="w-full max-w-6xl flex flex-col items-center justify-center gap-4 py-2 flex-1">
        {/* ROW 1: Vùng Tiêu đề & Bullet points (Hiển thị ở Step 1, 2, 3) */}
        <div
          className={`flex w-full gap-4 transition-all duration-1000 ease-out flex-shrink-0
          ${step === 1 ? "flex-col items-center scale-105" : "flex-col md:flex-row items-center justify-start scale-100"}`}
        >
          {/* Nút Khái Quát tròn */}
          <button
            onClick={handleNextStep}
            title={step < 3 ? "Bấm để xem tiếp" : "Bấm để làm lại"}
            ...
            ${step === 1 ? "w-48 h-48 sm:w-56 sm:h-56 text-2xl sm:text-3xl md:w-64 md:h-64 md:text-4xl" : "w-24 h-24 sm:w-28 sm:h-28 text-lg sm:text-xl"}`}
          >
            <span className="drop-shadow-md">Khái quát</span>
          </button>

          {/* Nội dung Khái Quát (Bullet points) - Hiện ở Step 2 & 3 */}
          <div
            ...
            ${step >= 2 ? "opacity-100 scale-100 md:translate-x-0 translate-y-0" : "opacity-0 scale-95 md:-translate-x-16 -translate-y-8 absolute pointer-events-none"}`}
          >
        ...
        {/* ROW 2: 3 ô nội dung chi tiết (General Details) - Hiện ở Step 3 */}
        <div
          className={`w-full flex flex-col md:flex-row gap-2 sm:gap-3 transition-all duration-[900ms] ease-out flex-shrink-0
          ${step === 3 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10 absolute pointer-events-none"}`}
        >
        ...
        {/* ROW 3: Mốc Thời Gian Container & Conclusion - Hiện ở Step 3 */}
        <div
          className={`w-full flex flex-col items-center transition-all duration-[1000ms] delay-100 ease-out flex-shrink-0 gap-3 lg:gap-4
          ${step >= 2 ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10 absolute pointer-events-none"}`}
        >
          {/* Heading */}
          ...
          <div className="hidden md:grid grid-cols-4 gap-3 lg:gap-4 w-full relative">
            ...
            <div
              className={`col-span-3 flex items-center gap-3 bg-white border-2 border-red-700 rounded-xl p-3 shadow-md transition-all mt-1
                ${step === 3 ? "opacity-100" : "opacity-0 translate-y-4 pointer-events-none"}`}
            >
            ...
            <div
              className={`col-span-1 flex items-center gap-3 bg-white border-2 border-red-700 rounded-xl p-3 shadow-md transition-all mt-1
                ${step >= 3 ? "opacity-100" : "opacity-0 translate-y-4 pointer-events-none"}`}
            >
              <div className="text-red-700 shrink-0 flex items-center justify-center">
                <svg
          ...
          <div className="md:hidden flex flex-col items-center gap-4 w-full">
            ...
            <div className="flex flex-col items-center gap-2 w-full">
              {khaiQuatData.milestones.slice(0, 3).map((item, index) => {
                const isActive = activeMilestones.includes(index);
                return (
                  <div
                    key={index}
              ...
              {/* Kết luận 1 */}
              <div className="flex items-start gap-2 bg-white border-2 border-red-700 rounded-xl p-3 shadow-md w-full max-w-[320px] mt-2">
            ...
            <div className="flex flex-col items-center w-full max-w-[280px] mt-4">
              <button
                ...
                className={`w-full h-10 rounded-t-xl font-extrabold text-sm flex items-center justify-center shadow transition-all duration-300 z-20 border border-b-0 border-red-700 ${activeMilestones.includes(3) ? "bg-red-900 text-white" : "bg-red-700 text-white"}`}
              >
                {khaiQuatData.milestones[3].time}
              </button>
              <div
                className={`w-full bg-gradient-to-b from-white to-orange-50 border border-t-0 border-orange-200 rounded-b-xl shadow-inner flex flex-col text-[13px] text-gray-800 font-medium transition-all duration-500 overflow-hidden ${activeMilestones.includes(3) ? "h-[120px] opacity-100 py-3 px-3" : "h-[30px] opacity-60"}`}
              >
                <div className="overflow-y-auto custom-scrollbar flex-1 pr-1">
                  <div
                    className={`transition-opacity duration-300 ${activeMilestones.includes(3) ? "opacity-100" : "opacity-0"}`}
                  >
                    {khaiQuatData.milestones[3].content}
                  </div>
                </div>
              </div>
              {/* Kết luận 2 */}
              <div className="flex items-start gap-2 bg-white border-2 border-red-700 rounded-xl p-3 shadow-md w-full mt-3">
                <div className="text-red-700 shrink-0 mt-0.5">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="18"
